...
//   { id, navn, type: 'L'|'S'|'U',
//     hverdag: null (aldrig) | [fra, til],  weekend: null | [fra, til],  ikkeDage: [0-4],
...
(function (root) {
  ...
  function tilModel(C) {
    ...
    const tidFra = (v, luk) => v === null ? null : v === undefined ? [AABEN, luk] : [v[0] == null ? AABEN : v[0], v[1] == null ? luk : v[1]];
    return C.personer.map(p => {
      ...
      return {
        ...
        hverdag: tidFra(t.hverdag, LUK_HVERDAG), weekend: tidFra(t.weekend, LUK_WEEKEND), ikkeDage: (t.ikkeDage || []).slice(),
  ...
  function fraModel(model, C) {
    const tidTil = (v, luk) => v === null ? null : (v[0] == AABEN && v[1] == luk) ? undefined : [v[0] == AABEN ? null : v[0], v[1] == luk ? null : v[1]];
    const gammel = {}; (C.personer || []).forEach(p => { gammel[p.id] = p; });
    const personer = model.map(m => {
      const tid = {};
      const hv = tidTil(m.hverdag, LUK_HVERDAG), we = tidTil(m.weekend, LUK_WEEKEND);
      if (hv !== undefined) tid.hverdag = hv;
      if (we !== undefined) tid.weekend = we;
      if (m.ikkeDage.length) tid.ikkeDage = m.ikkeDage.slice().sort();
    ...
    const hverdagsDage = m => [0, 1, 2, 3, 4].filter(d => m.hverdag && !m.ikkeDage.includes(d));
  ...
  function beskriv(m) {
    ...
    if (m.kunFaste) dele.push('kun de faste vagter');
      ...
      const kan = [];
      if (m.hverdag) kan.push(`hverdage ${kl(m.hverdag[0])}–${kl(m.hverdag[1])}` + (m.ikkeDage.length ? ` (ikke ${liste(m.ikkeDage.map(d => DAG[d]))})` : ''));
      else kan.push('aldrig hverdage');
      if (m.weekend) kan.push(`weekend ${kl(m.weekend[0])}–${kl(m.weekend[1])}`);
      else kan.push('aldrig weekend');
      dele.push('kan arbejde ' + liste(kan));
    }
    return dele.join('; ');
  }
