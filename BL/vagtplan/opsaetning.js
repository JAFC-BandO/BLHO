...
//   { id, navn, email, timeloen (kr./time eller null), type: 'L'|'S'|'F'|'U',
...
(function (root) {
  ...
  function tilModel(C) {
    ...
    return C.personer.map(p => {
      ...
      return {
        id: p.id, navn: p.navn, email: p.email || '', timeloen: typeof p.timeloen == 'number' ? p.timeloen : null, type: p.type,
  ...
  function fraModel(model, C) {
    ...
    const personer = model.map(m => {
      ...
      delete p.tid; delete p.kunFaste; delete p.email; delete p.timeloen;
      if (m.email) p.email = m.email;
      if (m.timeloen != null) p.timeloen = m.timeloen;
